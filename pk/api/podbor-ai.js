  ...
  try {
    ...
    if (action === 'next') {
      ...
      sys = [
        ...
        'Верни СТРОГО JSON: {"analysis":"...","whys":{"<id>":"..."},"plan":[{"when":"когда принимать","items":"названия продуктов из набора"}]}.',
        'analysis — РАЗВЁРНУТЫЙ персональный разбор на 5–8 предложений, разбитый на 2–3 абзаца через \\n\\n.',
        'Структура analysis: первый абзац — что мы поняли про клиента из его ответов (цель, нагрузка, режим, сон, питание), обращайся по имени, если оно есть.',
        'Второй абзац — где именно у него узкое место и почему набор собран так, а не иначе.',
        'Третий абзац — что делать в первую очередь и чего ждать: первые изменения обычно через 2–3 недели регулярного приёма, устойчивый результат — через 2–3 месяца.',
        'Пиши человеческим языком, без терминов вроде «нутриенты» и «дефицит макронутриентов», без канцелярита.',
