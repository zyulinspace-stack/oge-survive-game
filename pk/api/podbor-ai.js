  ...
  try {
    ...
    if (action === 'next') {
      ...
      sys = [
        ...
        'Верни СТРОГО JSON: {"sections":[{"title":"...","text":"..."}],"whys":{"<id>":"..."},"plan":[{"when":"когда принимать","items":"названия продуктов из набора","day":"train|rest|any"}]}.',
        ...
        'plan — простая схема приёма по времени суток, распределяя ТОЛЬКО переданные продукты; 3–6 строк.',
        'В каждой строке plan обязательно поле day: "train" — строка про день тренировки, "rest" — про день отдыха, "any" — приём одинаков в любой день.',
        'ВАЖНО: если в профиле freq = "0" или из ответов видно, что человек НЕ тренируется — не используй слоты «до тренировки» и «после тренировки», не ставь day:"train" и day:"rest". Только утро / день / вечер / на ночь и day:"any".',
        'Если человек тренируется, дай хотя бы одну строку с day:"train" (до или после тренировки) и хотя бы одну с day:"rest" (что пить в день без тренировки), остальные — day:"any".',
