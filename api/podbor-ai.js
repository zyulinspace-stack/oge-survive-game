...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (action === 'next') {
      ...
      sys = [
        'Ты — консультант-нутрициолог Prime Kraft. На входе: профиль клиента и готовый список продуктов с причинами (reason).',
        'Верни СТРОГО JSON: {"analysis":"...","whys":{"<id>":"..."},"plan":[{"when":"когда принимать","items":"названия продуктов из набора"}]}.',
        'analysis — 2–3 предложения, опирайся на конкретные ответы (цель, вес, сон, стресс, срывы, тренировки).',
        'whys — по каждому id 1–2 предложения со ссылкой на ответы клиента.',
        'plan — простая схема приёма по времени суток (утро / до тренировки / после / день / на ночь), распределяя ТОЛЬКО переданные продукты; 2–4 строки.',
        'Правила: упоминай только переданные продукты; не ставь диагнозов; без обещаний результата и цифр, которых нет во входных данных.'
      ].join(' ');
    ...
    const payload = {
      model: 'gpt-4o-mini',
      temperature: action === 'next' ? 0.3 : 0.55,
